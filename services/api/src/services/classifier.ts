...
export type Suggestion = { tagId: string; score: number; level: 'high' | 'mid' | 'low' };
export type ExplainItem = { tagId: string; reasons: string[] };

/** 规则置信度阈值：低于此值时尝试 LLM 兜底 */
const RULE_CONFIDENCE_THRESHOLD = 0.65;

/** 内置语义扩展：标签名不在文本时，用相关词提升匹配（如「安全检查」→「安全生产」） */
const TAG_SEMANTIC_EXPANSIONS: Record<string, string[]> = {
  安全生产: [
...
function levelFrom(scores: number[], idx: number): 'high' | 'mid' | 'low' {
  const top = scores[0] ?? 0;
  const current = scores[idx] ?? 0;
  if (top <= 0.0001) return 'low';
  if (idx === 0 && current >= 0.75) return 'high';
  if (current / top >= 0.65) return 'mid';
  return 'low';
}
...
function buildInputTokenCounts(text: string): Record<string, number> {
  ...
  for (const p of hanParts) {
    ...
    for (let i = 0; i <= p.length - 3; i += 1) {
      ...
      if (i <= p.length - 4) longer.push(p.slice(i, i + 4));
    }
  }
  const inputCounts: Record<string, number> = {};
  for (const t of [...fromSimple, ...longer]) {
    if (t.length >= 2) inputCounts[t] = (inputCounts[t] ?? 0) + 1;
  }
  return inputCounts;
}

function ruleBasedClassify(
  tags: Array<{ id: string; name: string; path?: string; keywords: unknown; updatedAt: Date }>,
  text: string,
  recordProfiles: Map<string, { recordTokens: Record<string, number>; sampleSnippets?: string[] }>
): Array<{ tagId: string; score: number; reasons: string[]; updatedAt: Date; tagName?: string }> {
  ...
  const scored = tags
    .map((t) => {
      ...
      const manual =
        Array.isArray((t.keywords as any)?.manual) ? (((t.keywords as any).manual as string[]) ?? []) : [];
      // 用户配置的关键词权重高，确保智能归类时优先匹配
      for (const k of manual) weights[k.toLowerCase()] = Math.max(weights[k.toLowerCase()] ?? 0, 5);
      ...
      const counts =
        (t.keywords as any)?.counts && typeof (t.keywords as any).counts === 'object'
          ? ((t.keywords as any).counts as Record<string, number>)
          : {};
      ...
      let hitWeight = 0;
      const hits: Array<{ tok: string; w: number }> = [];
      ...
      for (const [tok, w] of Object.entries(weights)) {
        if (tok === tagNameLower && tagNameInText) continue; // 已处理
        if (inputCounts[tok]) {
    ...
    .sort((a, b) => {
      ...
      const pathA = tags.find((t) => t.id === (a as any).tagId)?.path ?? '';
      const pathB = tags.find((t) => t.id === (b as any).tagId)?.path ?? '';
      const depthA = pathA.split('.').length;
      const depthB = pathB.split('.').length;
      if (depthA !== depthB) return depthB - depthA; // 优先更深层级
      // 同层时：优先选标签名在文本中更靠前的（更贴合该段主题）
      const idxA = textLower.indexOf((a as any).tagName?.toLowerCase() ?? '');
      const idxB = textLower.indexOf((b as any).tagName?.toLowerCase() ?? '');
  ...
  if (scored.length === 0) {
    const fallback = [...tags]
      ...
      .slice(0, 3)
      .map((t) => ({ tagId: t.id, score: 0.1, reasons: ['冷启动兜底：最近更新的标签'] as string[], updatedAt: t.updatedAt }));
 ...
 */
async function getLeafTags(userId: string) {
  ...
  const parentIds = await prisma.tag
    ...
    .then((rows) => new Set(rows.map((r) => r.parentId).filter(Boolean) as string[]));
...
export async function classifyForUser(
  ...
  if (tags.length === 0) return { suggestions: [], explain: [] };
  ...
  if (
    reflectionPrefix.test(text.trim()) &&
    knowledgeTag &&
    topTagFromRule &&
  ...
  const safetyKeywords =
    /隐患|隐患台账|隐患报告|隐患分析|年度隐患|安全工作|安全事项|联络单|安全检查|安全培训|隐患排查/;
  ...
  const textHasMultipleParts =
    (text.trim().includes(' ') || /[。；;]/.test(text.trim())) && text.trim().length > 15;
  ...
  const suggestions: Suggestion[] = scored.map((s, idx) => ({
    tagId: s.tagId,
    score: Number(s.score.toFixed(2)),
    level: levelFrom(scores, idx),
  }));

  const explain: ExplainItem[] = scored
