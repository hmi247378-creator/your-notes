...
export async function registerNoteRoutes(app: FastifyInstance) {
  app.post('/api/notes', { preHandler: requireAuth }, async (req, reply) => {
    ...
    if (!body.success) throw badRequest('Invalid note payload', body.error.flatten());
  ...
  app.post('/api/notes/ingest/preview', { preHandler: requireAuth }, async (req, reply) => {
    ...
    if (!body.success) throw badRequest('Invalid ingest preview payload', body.error.flatten());
    ...
    if (items.length === 0) throw badRequest('Empty text');

    const previewItems: Array<{ text: string; tagIds: string[]; suggestions: Array<{ tagId: string; score: number; level: string }> }> = [];

    for (const item of items) {
      const contentPlain = toPlainText(item);
      let tagIds: string[];
      let suggestions: Array<{ tagId: string; score: number; level: string }>;
  ...
  app.post('/api/notes/ingest/confirm', { preHandler: requireAuth }, async (req, reply) => {
    ...
    const body = schema.safeParse(req.body);
    if (!body.success) throw badRequest('Invalid ingest confirm payload', body.error.flatten());

    const created: Array<{ id: string; text: string; tagIds: string[] }> = [];
    ...
    for (const item of body.data.items) {
      ...
      if (tagIds.length) await strengthenTagsFromText({ userId, tagIds, text: contentPlain });
  ...
  app.post('/api/notes/ingest', { preHandler: requireAuth }, async (req, reply) => {
    ...
    const body = schema.safeParse(req.body);
    if (!body.success) throw badRequest('Invalid ingest payload', body.error.flatten());
    ...
    for (const item of items) {
      ...
      let suggestionsForLog: Array<{ tagId: string; score: number; level: string }>;
      if (preferredTagId) {
        tagIds = [preferredTagId];
        suggestionsForLog = [];
      } else {
  ...
  app.get('/api/notes/tag-counts', { preHandler: requireAuth }, async (req, reply) => {
    ...
    for (const r of rows) counts[r.tagId] = r._count.tagId;
  ...
  app.get('/api/notes', { preHandler: requireAuth }, async (req, reply) => {
    ...
    const batchRows: Row[] = batches.map((b) => {
      const tagSet = new Set<string>();
      for (const n of b.notes) for (const t of n.noteTags) tagSet.add(t.tagId);
      return {
        id: `batch:${b.id}`,
        ...
        archived: false,
        _sortKey: (b.recordedAt ?? b.createdAt) as Date,
      };
    });

    // 无批次的历史记录：按记录日期分组，同一天的多条合并为一条显示（模拟原始录入）
    const singleNoteRows: Row[] = [];
    const byDate = new Map<string, typeof singleNotes>();
    ...
    for (const [dateStr, notes] of byDate) {
      const first = notes[0]!;
      const sortKey = (first.recordedAt ?? first.createdAt) as Date;
      if (notes.length === 1) {
        singleNoteRows.push({
          id: first.id,
        ...
        const tagSet = new Set<string>();
        for (const n of notes) for (const t of n.noteTags) tagSet.add(t.tagId);
        singleNoteRows.push({
          id: `date:${dateStr}`,
  ...
  app.get('/api/notes/:id', { preHandler: requireAuth }, async (req, reply) => {
    const userId = req.user.userId;
    const noteId = z.string().uuid().parse((req.params as any).id);
  ...
  app.delete('/api/notes/by-date/:dateStr', { preHandler: requireAuth }, async (req, reply) => {
    const userId = req.user.userId;
    const dateStr = (req.params as any).dateStr as string;
    if (!/^\d{4}-\d{2}-\d{2}$/.test(dateStr)) throw badRequest('Invalid date format, use YYYY-MM-DD', null);
    ...
    for (const n of notes) await logChange({ userId, entityType: 'note', entityId: n.id, op: 'delete' });
  ...
  app.delete('/api/notes/batch/:batchId', { preHandler: requireAuth }, async (req, reply) => {
    const userId = req.user.userId;
    const batchId = z.string().uuid().parse((req.params as any).batchId);
  ...
  app.post('/api/notes/batch', { preHandler: requireAuth }, async (req, reply) => {
    ...
    const body = schema.safeParse(req.body);
    if (!body.success) throw badRequest('Invalid batch payload', body.error.flatten());
