...
function buildTree(tags: Array<Omit<TagNode, 'children'>>): TagNode[] {
...
async function assertDepthOk(userId: string, parentId: string | null, movingTagId?: string) {
  const parent = parentId
    ? await prisma.tag.findFirst({ where: { id: parentId, userId }, select: { depth: true, path: true } })
    : null;
  if (parentId && !parent) throw notFound('Parent tag not found');

  const baseDepth = (parent?.depth ?? 0) + 1;
  if (!movingTagId) {
    if (baseDepth > 7) throw badRequest('Tag depth exceeds limit (<=7)');
  ...
  if (!current) throw notFound('Tag not found');
  ...
  if (maxDepthAfterMove > 7) throw badRequest('Move would exceed tag depth limit (<=7)');
  return { parentPath: parent?.path ?? null, depth: baseDepth };
}
...
export async function registerTagRoutes(app: FastifyInstance) {
  ...
  app.post('/api/tags', { preHandler: requireAuth }, async (req, reply) => {
    ...
    if (!body.success) throw badRequest('Invalid tag payload', body.error.flatten());
    ...
    const tag = await prisma.tag.create({
      data: { userId, name: body.data.name, color: body.data.color, parentId, path, depth, keywords: keywords as any },
      select: { id: true, path: true, depth: true },
    });
  ...
  app.patch('/api/tags/:id', { preHandler: requireAuth }, async (req, reply) => {
    const userId = req.user.userId;
    const tagId = z.string().uuid().parse((req.params as any).id);
    ...
    if (body.data.keywords) {
      ...
      await prisma.tag.update({ where: { id: tagId }, data: { keywords: nextKeywords as any } });
    }
  ...
  app.post('/api/tags/merge', { preHandler: requireAuth }, async (req, reply) => {
    ...
    if (!body.success) throw badRequest('Invalid merge payload', body.error.flatten());

    const target = await prisma.tag.findFirst({ where: { id: body.data.targetTagId, userId } });
    if (!target) throw notFound('Target tag not found');
  ...
  app.delete('/api/tags/:id', { preHandler: requireAuth }, async (req, reply) => {
    ...
    if (childCount > 0) throw badRequest('Tag has children; delete blocked in Phase 1');

    const noteCount = await prisma.noteTag.count({ where: { tagId } });
    if (noteCount > 0) throw badRequest('Tag has notes; delete blocked in Phase 1');
