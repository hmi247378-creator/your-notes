...
export function splitIngestText(raw: string): string[] {
  ...
  if (cleaned.length === 1 && cleaned[0]!.length >= 15) {
    if (!LABEL_COLON_PREFIX.test(cleaned[0]!)) {
      ...
      if (bySentence.length > 1) cleaned = bySentence;
    }
  }
...
function mergeLabeledBlocks(lines: string[]): string[] {
  const out: string[] = [];
  let i = 0;
  const startsWithNumber = (s: string) => /^\s*\d+\s*[、.．)\]\s，,]\s*/.test(s);
...
function splitByNumbered(text: string): string[] {
  // 按编号前缀切：保留内容
