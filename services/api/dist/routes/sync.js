...
export async function registerSyncRoutes(app) {
    app.post('/api/sync/push', { preHandler: requireAuth }, async (req, reply) => {
        ...
        if (!body.success)
            throw badRequest('Invalid sync payload', body.error.flatten());
        let accepted = 0;
...
async function applyNoteChange(userId, change) {
    ...
    if (!parsed.success)
        throw badRequest('Invalid note payload in sync', parsed.error.flatten());
...
async function applyTagChange(userId, change) {
    ...
    const parsed = payloadSchema.safeParse(change.payload);
    if (!parsed.success)
        throw badRequest('Invalid tag payload in sync', parsed.error.flatten());
